...
fn classify_api(change: &Change) -> (Severity, &'static str) {
    ...
    if matches!(
        change.change_type,
        ChangeType::ValueRemoved | ChangeType::ObservationRemoved
    ) {
        return (
            Severity::Breaking,
            "previously observed API behavior was removed",
        );
    }
    if matches!(
        change.change_type,
        ChangeType::ValueAdded | ChangeType::ObservationAdded
    ) {
        return (
            Severity::Minor,
            "API behavior was added; additions are compatible by default",
        );
    }
...
fn classify_accessibility(change: &Change) -> (Severity, &'static str) {
    ...
    if path.ends_with("/role")
        && value_text(&change.before) == Some("button")
        && value_text(&change.after) == Some("generic")
    {
        return (
            Severity::Breaking,
            "interactive semantic role regressed from button to generic",
        );
    }
    ...
    if matches!(change.change_type, ChangeType::ObservationRemoved) {
        return (
            ...
            "accessibility observation disappeared",
        );
    }
...
fn classify_cli(change: &Change) -> (Severity, &'static str) {
    ...
    (
        Severity::Significant,
        "CLI observable output or filesystem behavior changed",
    )
}
...
fn classify_browser(change: &Change) -> (Severity, &'static str) {
    ...
    if matches!(change.change_type, ChangeType::ObservationRemoved) {
        return (
            Severity::Significant,
            "previously observed browser behavior disappeared",
        );
    }
    if path.contains("dialogs")
        && array_len(&change.before).is_some_and(|len| len > 0)
        && array_len(&change.after) == Some(0)
    {
        return (
            Severity::Breaking,
            "browser confirmation dialog disappeared",
        );
    }
    if path.contains("visibletext") || path.ends_with("/text") || path.contains("copy") {
        return (
            Severity::Minor,
            "visible copy changed without a stronger deterministic signal",
        );
    }
        ...
        && change.after.as_ref().and_then(Value::as_bool) == Some(false)
    {
        return (
            Severity::Breaking,
            "destructive confirmation behavior was removed",
        );
    }
    (
        Severity::Significant,
        "browser interaction or state changed",
    )
}
...
fn classify_events(change: &Change) -> (Severity, &'static str) {
    ...
    if matches!(change.change_type, ChangeType::ObservationRemoved) {
        return (
            Severity::Breaking,
            "previously emitted event behavior disappeared",
        );
    }
...
fn classify_performance(
    ...
    if let (Some(before), Some(after)) = (as_f64(&change.before), as_f64(&change.after)) {
        if before > 0.0 && after > before {
            ...
            if increase >= config.performance_regression_percent {
                return (
                    Severity::Significant,
                    "performance regression exceeded configured threshold",
                );
            }
        }
    }
    (
        Severity::Minor,
        "performance changed below significant regression threshold",
    )
}
