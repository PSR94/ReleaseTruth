...
pub fn render(format: ReportFormat, comparison: &Comparison, score: &ScoreBreakdown) -> Result<String> {
    Ok(match format {
        ReportFormat::Text => render_text(comparison, score),
        ReportFormat::Json => serde_json::to_string_pretty(&ComparisonReport { score, comparison })?,
...
fn counts(score: &ScoreBreakdown) -> BTreeMap<Severity, usize> {
    ...
    for severity in [Severity::Breaking, Severity::Significant, Severity::Minor, Severity::Expected] {
        result.insert(severity, *score.counts_by_severity.get(&severity).unwrap_or(&0));
    }
    result
}
...
fn render_markdown(comparison: &Comparison, score: &ScoreBreakdown) -> String {
    ...
    out.push_str("| Severity | Surface | Behavior | Path | Reason |\n| --- | --- | --- | --- | --- |\n");
    for change in &comparison.changes {
        out.push_str(&format!(
            "| {} | {} | `{}` | `{}` | {} |\n",
            change.severity,
...
fn render_html(comparison: &Comparison, score: &ScoreBreakdown) -> String {
    ...
    let rows: String = comparison
        ...
        .collect();
    format!(r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ReleaseTruth Behavioral Diff</title><style>
...
fn compact(value: &Option<serde_json::Value>) -> String {
    value.as_ref().map_or_else(|| "∅".to_owned(), |value| value.to_string())
}

fn pretty(value: &Option<serde_json::Value>) -> String {
    value.as_ref().map_or_else(|| "∅".to_owned(), |value| serde_json::to_string_pretty(value).unwrap_or_else(|_| value.to_string()))
}

fn md(value: &str) -> String { value.replace('|', "\\|").replace('`', "\\`") }
fn xml(value: &str) -> String { value.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;").replace('"', "&quot;").replace('\'', "&apos;") }
fn html(value: &str) -> String { xml(value) }
...
mod tests {
    ...
    fn fixture() -> (Comparison, ScoreBreakdown) {
        let comparison = Comparison {
            ...
            changes: vec![Change {
                id: "chg-1".into(), surface: Surface::Api, observation_id: "POST /orders".into(), path: "/status".into(),
                change_type: ChangeType::ValueChanged, before: Some(json!(400)), after: Some(json!(422)), severity: Severity::Breaking,
                confidence: 1.0, evidence: vec!["http-1".into()], classification_reason: "HTTP status contract changed".into()
            }]
        };
        let score = ScoreBreakdown { score: 80.0, total_penalty: 20.0, penalties_by_severity: BTreeMap::new(), penalties_by_surface: BTreeMap::new(), counts_by_severity: BTreeMap::from([(Severity::Breaking, 1)]) };
        (comparison, score)
    }
    ...
    fn all_report_formats_render() {
        ...
        for format in [ReportFormat::Text, ReportFormat::Json, ReportFormat::Markdown, ReportFormat::Junit, ReportFormat::Sarif, ReportFormat::Html] {
